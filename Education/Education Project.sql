...
--Minimum, maximum, and average median_household_income of the nation? For each state?
SELECT MIN(median_household_income), AVG(median_household_income), MAX(median_household_income) FROM census
...
--The view shows only schools that have data for math and reading proficiencies AND have median household income for that zip code.
SELECT * FROM income;
...
SELECT AVG(pct_proficient_math), AVG(pct_proficient_reading) FROM income;
--On average, students perform better on the reading exam
--The query below produces the 7 States, Iowa, Indiana, Maryland, South Carolina, Tennessee, Texas, and Wyoming where average state math scores are higher than the average state reading scores
